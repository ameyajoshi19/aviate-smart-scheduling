...
export function SwipeableCard({ task, onPress, onComplete, onDelete, onReschedule, index }: SwipeableCardProps) {
    ...
    <Animated.View entering={FadeInRight.delay(index * 60).springify()} style={styles.wrapper}>
      <Swipeable
        ...
        onSwipeableOpen={(dir) => {
          ...
          if (dir === "right") {
            // right side opened = user swiped LEFT = DELETE
            Haptics.notificationAsync(Haptics.NotificationFeedbackType.Warning);
            onDelete();
          } else {
            // left side opened = user swiped RIGHT = RESCHEDULE
            Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Medium);
            onReschedule();
          }
        }}
